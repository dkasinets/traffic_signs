...
def croppedOnlyProhibitoryTransformedCNNModel(train_df, test_df, valid_df, OUTPUT_DIR_TRAIN, OUTPUT_DIR_TEST, OUTPUT_DIR_VALID, k_fold = False, grayscale = False, debug = False):
    """
    ...
    # output layer
    class_id_head = layers.Dense(5, activation = 'softmax')(x)
    ...
    model_info_dict = {'method': 'croppedOnlyProhibitoryTransformedCNNModel', 'image_size': f"{image_size}x{image_size}", 
...
def croppedOnlyProhibitoryCNNModel(train_df, test_df, valid_df, OUTPUT_DIR_TRAIN, OUTPUT_DIR_TEST, OUTPUT_DIR_VALID, k_fold = False, grayscale = False, debug = False):
    """
        Goal: Predict 5 Road Sign Classes.
        The total number is 5, because we only consider Prohibitory Signs.
        The speed signs are all aggregated as ClassID = 999.
    ...
    model_info_dict = {'method': 'croppedOnlyProhibitoryCNNModel', 'image_size': f"{image_size}x{image_size}", 
                       'channels': channels, 'epochs': epochs, 
...
def runCroppedOnlyProhibitory(oversample = False, apply_transform = False, k_fold = False, grayscale = False, save_output = True, export_input_dataframes = False):
    ...
    if apply_transform:
        prediction_df, evaluate_info_df = croppedOnlyProhibitoryTransformedCNNModel(filtered_train_df, filtered_test_df, filtered_val_df, OUTPUT_DIR_TRAIN_CROPPED_PROHIB_ONLY, OUTPUT_DIR_TEST_CROPPED_PROHIB_ONLY, OUTPUT_DIR_VALID_CROPPED_PROHIB_ONLY, k_fold, grayscale, debug = True)
    else:
        prediction_df, evaluate_info_df = croppedOnlyProhibitoryCNNModel(filtered_train_df, filtered_test_df, filtered_val_df, OUTPUT_DIR_TRAIN_CROPPED_PROHIB_ONLY, OUTPUT_DIR_TEST_CROPPED_PROHIB_ONLY, OUTPUT_DIR_VALID_CROPPED_PROHIB_ONLY, k_fold, grayscale, debug = True)
    
    runtime = tmr.ShowTime() # End timer.
    evaluate_info_df['runtime'] = str(runtime)

    output_name = f"cropped_only_Prohibitory{'_applied_transform' if apply_transform else ''}" # It can be part of a file name or folder name
    if save_output:
        writeToExcel(prediction_df, evaluate_info_df, PROHIBITORY_ONLY_PRESENT_EXCEL, OUTPUT_DIR_TEST = None, name = output_name)
        saveMisclassifiedImages(prediction_df, actual_col = '(Actual) ClassID', predicted_col = '(Predicted) ClassID', filename_col = 'Image Filename', input_test_dir = OUTPUT_DIR_TEST_CROPPED_PROHIB_ONLY, output_img_dir = PROHIBITORY_ONLY_PRESENT_IMG, name = output_name)
    if export_input_dataframes: 
        exportTrainTestValidDataframes(filtered_train_df, filtered_test_df, filtered_val_df, PROHIBITORY_ONLY_PRESENT_EXCEL)
